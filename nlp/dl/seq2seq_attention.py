...
class Seq2Seq(torch.nn.Module):
    def __init__(self, enc_v_dim, dec_v_dim, emb_dim, units, max_pred_len, start_token, end_token):
        ...
        self.encoder = torch.nn.LSTM(emb_dim, units, 1, batch_first=True)

        # decoder
        self.dec_embeddings = torch.nn.Embedding(dec_v_dim, emb_dim)
        self.attn = torch.nn.Linear(units, units)
        self.decoder_cell = torch.nn.LSTMCell(emb_dim, units)
        self.decoder_dense = torch.nn.Linear(units * 2, dec_v_dim)
    ...
    def inference(self, x):
        ...
        for i in range(self.max_pred_len):
            ...
            attn_weight = torch.nn.functional.softmax(attn_prod, dim=1)  # [n, step, 1]
    ...
    def train_logit(self, x, y):
        ...
        dec_emb_in = dec_emb_in.permute(1, 0, 2)  # [step, , emb_dim]
        output = []
    ...
    def step(self, x, y):
        ...
        dec_out = y[:, 1:]
        loss = torch.nn.functional.cross_entropy(logit.reshape(-1, self.dec_v_dim),
                                                 dec_out.reshape(-1).type(torch.LongTensor))
        loss.backward()
        self.optimizer.step()
