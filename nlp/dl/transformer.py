...
from nlp.tools.utils import DateData, pad_zero
...
class MultiHead(torch.nn.Module):
    def __init__(self, n_head, model_dim, drop_rate):
        ...
        self.wq = torch.nn.Linear(model_dim, n_head * self.head_dim)
        self.wk = torch.nn.Linear(model_dim, n_head * self.head_dim)
        self.wv = torch.nn.Linear(model_dim, n_head * self.head_dim)

        self.o_dense = torch.nn.Linear(model_dim, model_dim)
        self.o_drop = torch.nn.Dropout(drop_rate)
    ...
    def forward(self, q, k, v, mask, training):
        ...
        # split by head
        query = self.split_heads(query)  # [n, n_head, step, h_dim]
    ...
    def scaled_dot_product_attention(self, q, k, v, mask=None):
        ...
        self.attention = torch.nn.Softmax(dim=-1)(score)
...
class PositionWiseFFN(torch.nn.Module):
    def __init__(self, model_dim, dropout):
        super().__init__()
        dff = model_dim * 4
        self.l = torch.nn.Linear(model_dim, dff)
        self.o = torch.nn.Linear(dff, model_dim)
        self.dropout = torch.nn.Dropout(dropout)
        self.layer_norm = torch.nn.LayerNorm(model_dim)

    def forward(self, x):
        o = torch.nn.ReLU()(self.l(x))
        o = self.o(o)
        o = self.dropout(o)
...
class DecoderLayer(torch.nn.Module):
    def __init__(self, n_head, model_dim, drop_rate):
        super().__init__()
        self.mh = torch.nn.ModuleList([MultiHead(n_head, model_dim, drop_rate) for _ in range(2)])
...
class PositionEmbedding(torch.nn.Module):
    def __init__(self, max_len, emb_dim, n_vocab):
        ...
        self.embeddings = torch.nn.Embedding(n_vocab, emb_dim)
...
class Transformer(torch.nn.Module):
    def __init__(self, n_vocab, max_len, n_layer=6, emb_dim=512, n_head=8, drop_rate=0.1, padding_idx=0):
        ...
        self.o = torch.nn.Linear(emb_dim, n_vocab)
        self.opt = torch.optim.Adam(self.parameters(), lr=2e-3)

    def forward(self, x, y, training=None):
        x_embed, y_embed = self.embed(x), self.embed(y)  # [n, step, emb_dim]
    ...
    def step(self, x, y):
        ...
        loss = torch.nn.CrossEntropyLoss()(logits.reshape(-1, self.dec_v_emb), y[:, 1:].reshape(-1))
        loss.backward()
        self.opt.step()
    ...
    def translate(self, src, v2i, i2v):
        ...
        target = torch.from_numpy(
            pad_zero(np.array([[v2i["<GO>"], ] for _ in range(len(src))]), self.max_len + 1)).to(device)
...
def train(emb_dim=32, n_layer=3, n_head=4):
    ...
    for i in range(100):
        for batch_idx, batch in enumerate(loader):
            ...
            bx, by = torch.from_numpy(pad_zero(bx, max_len=MAX_LEN)).type(torch.LongTensor).to(
                device), torch.from_numpy(pad_zero(by, MAX_LEN + 1)).type(torch.LongTensor).to(device)
